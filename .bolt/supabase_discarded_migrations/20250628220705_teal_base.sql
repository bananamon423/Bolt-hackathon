/*
  # Token Management System Functions

  This migration creates comprehensive token management functions:
  1. consume_tokens - Deducts tokens from user profiles with validation
  2. reset_monthly_tokens - Resets user tokens based on subscription plan
  3. get_user_tokens - Retrieves current token balance and plan info
  ...
  - Functions validate user existence and token availability
  - Comprehensive error handling and logging
*/

-- Drop all possible variations of these functions to ensure clean slate
DROP FUNCTION IF EXISTS consume_tokens CASCADE;
DROP FUNCTION IF EXISTS reset_monthly_tokens CASCADE;
DROP FUNCTION IF EXISTS get_user_tokens CASCADE;
...
CREATE FUNCTION reset_monthly_tokens(p_user_id UUID)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
CREATE FUNCTION get_user_tokens(p_user_id UUID)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
CREATE FUNCTION update_user_subscription(

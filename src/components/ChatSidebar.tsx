...
export function ChatSidebar({ 
  ...
  return (
    ...
    style={{
      ...
      backgroundSize: collapsed ? '100% 100%' : '100% 100%',
      ...
      <div className="h-full flex flex-col relative z-10" style={{ paddingLeft: collapsed ? '8px' : '20px', paddingTop: '60px' }}>
        ...
        <div className="absolute top-2 left-4 right-4 flex items-center justify-between">
          {!collapsed && (
            <div className="flex items-center gap-2">
              <div className="bg-gradient-to-br from-amber-800 to-amber-900 p-2 rounded-lg shadow-md border border-amber-700">
                <MessageSquare className="w-5 h-5 text-amber-100" />
              </div>
              <h1 className="text-lg font-black text-amber-900 drop-shadow-md tracking-wide">AI WORKSPACE</h1>
            </div>
          )}
          <button
            onClick={onToggleCollapse}
            className="p-2 hover:bg-amber-200/60 rounded-lg transition-colors text-amber-800 shadow-sm border border-amber-300/50 bg-amber-100/40"
          >
        ...
        {!collapsed && profile && (
          <div className="px-4 pb-4 mt-8">
            <div className="flex items-center gap-3 bg-teal-500 p-3 rounded-lg shadow-lg border-2 border-teal-600">
              <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center text-teal-600 font-black text-lg shadow-md border-2 border-teal-200">
                {profile.username?.[0]?.toUpperCase() || 'U'}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-black text-white uppercase tracking-wider drop-shadow-sm">
        ...
        <div className="flex-1 overflow-y-auto px-4 space-y-2" style={{ maxHeight: 'calc(100vh - 300px)' }}>
          {chats.map((chat, index) => (
            ...
            >
              <div className={`p-3 rounded-lg font-black text-white text-center uppercase tracking-wider shadow-md border-2 transform hover:scale-105 transition-transform ${
              ...
              } ${collapsed ? 'text-xs p-2' : 'text-sm'}`}>
                {!collapsed && (
                  <>
                    <div className="flex items-center justify-between">
                      <span className="truncate flex-1 drop-shadow-sm text-xs">
                ...
                {showChatMenu === chat.id && (
                  <div className="absolute top-full right-0 mt-1 bg-white border-2 border-gray-300 rounded-lg shadow-xl z-10 min-w-32">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                      ...
                      className="w-full text-left px-3 py-2 text-sm text-red-600 hover:bg-red-50 flex items-center gap-2 rounded-lg font-semibold"
                    >
          ...
          <button
            onClick={onNewChat}
            className="w-full p-3 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-lg font-black uppercase tracking-wider shadow-md border-2 border-blue-800 hover:from-blue-700 hover:to-blue-800 transition-all transform hover:scale-105 flex items-center justify-center gap-2 shadow-blue-300/50"
          >
            <Plus className="w-5 h-5" />
            {!collapsed && <span className="text-sm drop-shadow-sm">New Chat</span>}
          </button>
        </div>

        {/* Bottom Actions - positioned at the bottom of the folder */}
        <div className="p-4 space-y-3" style={{ marginTop: 'auto' }}>
          ...
          {subscription && !collapsed && (
            <div className="mb-3">
              <TokenUsageIndicator
                ...
                className="bg-amber-50/95 border-2 border-amber-400/70 shadow-lg backdrop-blur-sm rounded-lg"
              />
            </div>
          )}
          ...
          {profile?.role === 'admin' && (
            <button
              ...
              className={`w-full text-amber-900 hover:text-amber-800 hover:bg-amber-200/60 rounded-lg transition-colors flex items-center gap-2 font-bold backdrop-blur-sm border border-amber-300/50 bg-amber-100/40 shadow-sm ${

...
export function MessageList({ 
  ...
  return (
    <div 
      ...
      style={{
        backgroundImage: 'url(/7mm_Grid_Paper_Blue_7mm_grid.jpg)',
        backgroundSize: 'auto',
    ...
    >
      ...
      {/* Messages container with relative positioning to appear above overlay */}
          ...
          if (isSystem) {
            return (
              <div key={message.id} className="flex justify-center group">
                <div className={`bg-white/90 backdrop-blur-sm text-gray-600 px-4 py-2 rounded-full text-sm relative shadow-sm border border-gray-200 ${
              ...
              >
                ...
                <div className={`flex flex-col ${isCurrentUser ? 'items-end' : 'items-start'} relative w-full`}>
                  <div className={'flex flex-row gap-2 items-center px-1 mb-2'}>
                    ...
                    {isAI && aiModelInfo && (
                      <div className="text-xs font-medium text-gray-700 mb-1 px-1 bg-white/60 rounded-full px-2 py-1">
                        {aiModelInfo.name}
                      </div>
                    )}
                </div>
                  <div
                    className={`px-4 py-3 rounded-2xl relative w-full shadow-sm backdrop-blur-sm ${
                      isDeleted 
                        ? 'opacity-50 border-2 border-dashed border-gray-300 bg-white/60'
                        : isAI
                        ? `bg-white/95 border-2 border-teal-300 rounded-bl-none text-gray-800`
                        : isCurrentUser
                  ...
                  <div className={`flex items-center gap-2 mt-1 text-xs text-gray-600 ${
                    ...
                    {!isCurrentUser && !isAI && message.profiles?.username && (
                      <span className="bg-white/70 rounded-full px-2 py-1">• {message.profiles.username}</span>
                    )}

                    {isDeleted && (
                      <div className="flex items-center gap-1 text-red-500 bg-white/70 rounded-full px-2 py-1">

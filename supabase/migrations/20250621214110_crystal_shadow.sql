/*
  # Fix PostgreSQL Function Parameter Error - Corrected Version

  1. Problem
    - PostgreSQL cannot change parameter names in existing functions
    - Functions have dependent policies that must be dropped first
    - Need to drop policies, then functions, then recreate everything

  2. Solution
    - Drop all dependent policies first
    - Drop all existing helper functions
    - Recreate functions with consistent parameter naming
    - Recreate all policies with corrected function calls
